    ...
    with open(csv_path, 'r', encoding='utf-8') as file:
        ...
        for row_num, row in enumerate(reader, start=2):
            try:
                ...
                accessibility = clean_string(row.get('Accessibility', ''))
                restroom_type = clean_string(row.get('Restroom Type', ''))
                changing_stations = clean_string(row.get('Changing Stations', ''))
